...
* `&` имеет больший приоритет, поэтому мы должны `|` вводить в скобки.

### Особенность использования двойных знаков и/или

### Двойное ИЛИ

```
((a + b) % 2 == 0) | ((a + b) % 10 == 3) - вычисляется левая часть, вычисляется правая часть, потом они проходят через ИЛИ

В случае ИЛИ - имеет ли смысл нам смотреть правую часть, если левая уже true? - НЕТ
```
* Мы ставим `a||b` - чтобы избежать второй проверки, когда левый операнд уже `true`, потому что независимо от правого операнда все выражение - `true`

* Пример с деньгами:

```
Вы подходите к магазину. Просите Snikers. Он стоит 99$.

if (ХватаетНалички || ХватаетНаКарте) {
	Платите тем, что есть
}
```
...
```
((a + b) % 2 == 0) & ((a + b) % 10 == 3) - вычисляется левая часть, вычисляется правая часть, потом они прохлодят через И

В случае И - имеет ли смысл нам смотреть правую часть, если левая уже false? - НЕТ
```

* Мы ставим `a&&b` - чтобы избежать второй проверки, когда левый операнд уже `false`, потому что независимо от правого операнда все выражение - `false`

* Пример продуктами:

```
Вы подходите к магазину. Надо купить и курицу и яйца

if (ЕстьКурица && ЕстьЯйца) {
	Покупаем и то и то
}
```