...
public class Table { // реализация ассоциативного массива
    ...
    private static class Pair { // класс, описывающий пару ключ-значение
        private String key; // ключ
        private int value; // значение

        public Pair(String key, int value) { // конструктор, в который можно передавать ключ-значение
            this.key = key;
            this.value = value;
        }
    }

    public class Printer { // класс для печати таблицы
        private String separator; // разделитель ключа и значения для вывода в консоль

        public Printer(String separator) { // передаем конкретную таблицу данному принтеру
