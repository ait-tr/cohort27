...
const Products: FC = () => {
  const { user, isLoading: authIsLoading } = useAppSelector(state => state.auth)
  ...
  return (
    <div>
      {!authIsLoading && !user?.firstName && (<h1>Пройдите регистрацию</h1>)}
      {authIsLoading && <h1>Загрузка</h1>}
      {user?.firstName && !authIsLoading && (
        <>
          <ProductForm />
